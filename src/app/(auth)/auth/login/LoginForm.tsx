...
export default function LoginForm() {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        const maxAge = remember ? 60 * 60 * 24 * 30 : undefined; // 30 days if remember me
        document.cookie = `custom-auth-token=${token}; path=/${maxAge ? `; max-age=${maxAge}` : ""}`;
    ...
    return (
        <>
            ...
            <Badge className="px-4 py-3 text-sm text-[#664a0c] bg-[#fff8e8] mt-5 font-normal">
                <p>Use <span className="mx-1.5 font-bold">user@nexora.io</span>with password <span className="mx-1.5 font-bold">Secret1</span></p>
            </Badge>
        </>
    );
}
